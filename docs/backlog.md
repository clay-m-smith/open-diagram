...
Add a compact action row beneath the diagram controls and status:
...
- Offer file saving when image clipboard support is unavailable. Do not install
  clipboard utilities automatically or overwrite files without confirmation.
...
- Deterministic SVG rendering preserves the vertical numbered-node layout and
  explicit edge endpoints. PNG rasterization is loaded only when requested.
